...
const cssTokens = 'ArticleCards,Heading,Picture,ArticleMeta,ButtonTertiary';
const bgBoxToken = ',BgBox';

export const NewsListParagraph: StoryComponent = () => {
  const Container = boolean('Grey background', true) ? BgBox : 'div';

  let displayedCssTokens = cssTokens;
  if (Container !== 'div') {
    displayedCssTokens += bgBoxToken;
  }

  return (
    <>
      <style>{customStyles}</style>
      <Container className="NewsListParagraph">
      ...
      <HiddenTiger>
        CSS tokens:{' '}
        <a href={getCssBundleUrl(displayedCssTokens)}>
          {displayedCssTokens.replace(',', ', ')}
        </a>
      </HiddenTiger>
    </>
  );
...
NewsListParagraph.story = {
  parameters: {
    css: { tokens: cssTokens + bgBoxToken },
  },
};
