...
export const SeenEffect__fadeup = (childSelector = '') => {
  return css`
    ${SeenEffect__initial(childSelector)(
      css`
        opacity: 0;
        transform: translateY(200px);
      `
    )}
    ${SeenEffect__seen(childSelector)(
      css`
        opacity: 1;
        transform: none;
      `
    )}
    ${SeenEffect__transition(childSelector)(
      css`
        ${Transition__long}
